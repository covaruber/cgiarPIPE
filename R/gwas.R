gwas <- function(
  ...
  myrel <- readRDS(file.path(wd,"files_cleaned",paste0(markerDTfile)))
  ...
  for(iTrait in trait){ #  iTrait = trait[3]
    ...
    predictionsBindHitsList[[iTrait]] <- data.frame(analysisId=id, pipeline= paste(sort(unique(mydataSub$pipeline)),collapse=", "),
  ...
  return(paste("gs/gwas done:",idmes))
}
